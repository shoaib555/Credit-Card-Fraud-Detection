...
```{r}
#Load the required data and EDA
rm(list=ls())
library(data.table)
cc=fread("CC.csv",sep=",")
summary(cc)
...
## Scaling the data and applying SMOTE
```{r}
cc1=scale(cc[,-30])
...
r.ctrl=rpart.control(minisplit=50,minbucket=10,cp=0.0062,xval=10)
...
rf=randomForest(Class~.,data=tr,ntree=100,mtry=3,nodesize=10,importance=T)
print(rf)
plot(rf)
...
plot(perf,col="black",lty=2, lwd=2)
plot(perf,lwd=3,colorize = TRUE)
plot(perf,colorize=T,print.cutoffs.at=seq(0,1,0.2),text.adj=c(-0.5,1.7),main="ROC for KNN")
```
...
lda= train(Class ~ ., data=tr, method="lda",trControl = trainControl(method = "cv",number=10))
lda
ts$pred=predict(lda,ts,type="prob")[,"1"]
summary(ts$pred)
...
plot(perf,col="black",lty=2, lwd=2)
plot(perf,lwd=3,colorize = TRUE)
plot(perf,colorize=T,print.cutoffs.at=seq(0,1,0.2),text.adj=c(-0.2,1.7),main="ROC for LDA")
```
...
qd= train(Class ~ ., data=tr, method="qda",trControl = trainControl(method = "cv",number=10))
qd
ts$pred=predict(qd,ts,type="prob")[,"1"]
summary(ts$pred)
...
plot(perf,col="black",lty=2, lwd=2)
plot(perf,lwd=3,colorize = TRUE)
plot(perf,colorize=T,print.cutoffs.at=seq(0,1,0.2),text.adj=c(-0.2,1.7),main="ROC for QDA")
